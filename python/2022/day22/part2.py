...
def compute_curses(stdscr: curses._CursesWindow, s: str) -> int:
    ...
    if len(lines[0]) < 50:
        ...
        back_to_orig_dir: dict[
            Face, Callable[[support.Direction4], support.Direction4]
        ] = {  # noqa: E501
        ...
        def _grid(face: Face, grid: set[tuple[int, int]]) -> None:
            for yi in range(size):
                stdscr.addstr(
                    yi + offsets[face][1],
                    offsets[face][0],
                    "".join("#" if (xi, yi) in grid else "." for xi in range(size)),
                )
        ...
        if size >= 50:
            ...
            for yi in range(size):
                stdscr.addstr(
                    yi + 1,
                    1,
                    "".join("#" if (xi, yi) in coords[face] else "." for xi in range(size)),
                )
