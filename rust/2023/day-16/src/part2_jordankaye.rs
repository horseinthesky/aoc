...
pub fn process(
    ...
    let traversal = Traversal { layout, rows, cols };

    let mut traversals = Vec::with_capacity(500);

    for r in 0..traversal.rows {
        traversals.push((r, 0, RIGHT));
        traversals.push((r, traversal.cols - 1, LEFT));
    }

    for c in 0..traversal.cols {
        traversals.push((0, c, DOWN));
        traversals.push((traversal.rows - 1, c, UP));
    }

    let res = traversals
        .par_iter()
        .map(|(r, c, d)| traversal.clone().traverse(*r, *c, *d))
        .max()
        .unwrap();
